...
interface AddWordFormProps {
  onWordAdded: (word: NewWordPayload) => void;
  onCancel: () => void;
}
...
const AddWordForm: React.FC<AddWordFormProps> = ({ onWordAdded, onCancel }) => {
  ...
  const [duplicateMotifWord, setDuplicateMotifWord] = useState<string | null>(
    null
  );
  const [invalidMotifChars, setInvalidMotifChars] = useState(false);
  const [motifPartOfSpeechError, setMotifPartOfSpeechError] = useState<
    string | null
  >(null);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const getMotifPartOfSpeechError = (
    motif: string,
    partOfSpeech: string
  ): string | null => {
  ...
  useEffect(() => {
    const checkMotifExists = async () => {
      ...
      const partOfSpeechError = getMotifPartOfSpeechError(
        motif,
      ...
      setErrors((prev) => ({
        ...prev,
        motifPartOfSpeech: !!partOfSpeechError,
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrors((prev) => ({
          ...prev,
          duplicate: errorData.message === "This word already exists",
        }));
      }
    } catch (error) {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex flex-col space-y-2">
            ...
            {duplicateMotifWord && (
              <p className="text-red-500 text-sm">
                This motif is already assigned to &quot;{duplicateMotifWord}
                &quot;
              </p>
            )}
          </div>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) =>
              setNewWord({ ...newWord, mnemonic: e.target.value })
            }
